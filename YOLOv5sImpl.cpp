...
bool ObjectDetectionImpl::Initialize(const ObjectDetectionConfig& config)
{
    ...
    //ALOGD(__func__,"===========ssss=======================","");
    //if (!m_task->init(config.model_path, CPU)) return false;
    m_task->init(config.model_path, config.runtime);
    //ALOGD(__func__,"===========cscscsc=======================","");
...
bool ObjectDetectionImpl::PreProcess(const cv::Mat& image)
{
    ...
    if (m_task->getInputTensor(m_inputLayers[0]) == nullptr)    return false;
    ...
    input /= 255.0f;
    //ALOGD(__func__,"============nnnnnnnnn========================", "");
    return true;
}

bool ObjectDetectionImpl::Detect(const cv::Mat& image,std::vector<ObjectData>& results)
{
